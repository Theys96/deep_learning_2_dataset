...
from tensorflow.keras.layers import MaxPooling2D, Concatenate, LeakyReLU
...
class AdversarialAutoencoder():
    ...
    def build_encoder(self):
        ...
        #latent_repr = Input(mu[0] + tf.random.normal(K.shape(mu[0])) * K.exp(log_var[1] / 2)) #, shape=mu.shape)
        latent_repr = Lambda(lambda p: p[0] + K.random.normal(K.shape(p[0])) * K.exp(p[1] / 2), output_shape=lambda p: p[0])([mu, log_var])
        '''
    ...
    def load_data(self, dataset_path):
        ...
        for img_name in img_list:
            data[i,:,:,:] = cv2.imread(os.path.join(dataset_path, img_name))
            i += 1
        print("Done.")
        return data
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # Load the dataset
        X_train = self.load_data("C:\\Users\\thijs\\Documents\\Studie\\IIa\\Deep Learning\\deep_learning_2_dataset\\dataset\\")
        '''
